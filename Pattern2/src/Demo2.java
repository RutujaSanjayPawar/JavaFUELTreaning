...
class ETC extends Handler
{
    ...
    {
        if (request.getValue().equals("ETC"))
        {
            //if request is eligibal handle it
            System.out.println("handled by ETC:");
            System.out.println("\tRequest : "+ request.getDescription()+request.getValue());
        }
        else{
...
class CSE extends Handler
{
    public void handleRequest(Request request)
    {
        if(request.getValue().equals("CSE"))
